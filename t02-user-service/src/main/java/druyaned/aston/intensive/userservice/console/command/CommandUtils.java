...
public class CommandUtils {
    ...
    /**
     * Message of a bad input to be output.
     */
    public static String BAD_INPUT = "Bad input";

    /**
     * Verifies the given input, which must be not null and not empty, as well
     * as each its element, and the length of the input must be equal to the
     * given length.
     *
     * @param input to be verified
     * @param length expected length of the input, more than 0
     * @return null in case of accepted input, otherwise a violation message to
    ...
    /**
     * Returns a violation report as a string, including all violations; the set
     * must not be empty.
     *
     * @param set to produce the violation output; must not be empty
     * @return violation output of all violations
     */
    public static String violationOutput(
            Set<ConstraintViolation<UserEntity>> set) {

        StringBuilder sb = new StringBuilder();
        sb.append("Constraint violations:");
        for (ConstraintViolation<UserEntity> violation : set) {
    ...
    /**
     * Returns fields of the user through spaces to construct the output for a
     * command.
     *
     * @param user to construct the output from its fields
     * @return fields of the user through spaces
     */
    public static String outputUser(UserEntity user) {
        StringBuilder sb = new StringBuilder();
        sb
